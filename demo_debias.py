...
from bend_vlm.LLINT import get_INLP_P, get_debiased_embeddings_INLP
...
def load_embedding_dataset(ds_path, split='train'):
    embeddings_dataset = load_dataset("json", data_files=ds_path, split=split)
...
####################################################
_embeddings_dataset = load_embedding_dataset({split: f'data/{dataset_name}_{split}_featurized_{_MODEL_NAME}.jsonl' for split in ('train', 'validation')}, 
                                             split='validation')
####################################################
...
print("FOLD", len(fold_dict['3']['train_indices']))
####################################################

reference_embeddings_dataset = load_embedding_dataset({split: f'data/{dataset_name}_{split}_featurized_{_MODEL_NAME}.jsonl' for split in ('train', 'validation')}, 
                                             split='train')
target_embeddings_dataset = load_embedding_dataset({split: f'data/{dataset_name}_{split}_featurized_{_MODEL_NAME}.jsonl' for split in ('train', 'validation')}, 
                                             split='validation')
####################################################
if query_type == 'hair':
...
for query_class in query_classes:
    ...
    for k_fold in range(1):
        ...
        reference_embeddings_dataset = reference_embeddings_dataset
        target_embeddings_dataset = target_embeddings_dataset
        ...
        result_dict[query_class]['P0'][f'fold_{k_fold}'] = get_metrics(llint_embeddings.numpy(), query_class, att_to_debias, K, prior_for_metric, 
